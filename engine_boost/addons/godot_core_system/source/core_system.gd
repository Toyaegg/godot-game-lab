...
const DebugLogger = preload("./debug_logger/debug_logger.gd")
...
@onready var debug_logger : DebugLogger = _get_module("debug_logger"):								## 日志管理器
	get:
		if not debug_logger:
			debug_logger = _get_module("debug_logger")
		return debug_logger
...
func _get_module(module_id: StringName) -> Node:
	if not _modules.has(module_id):
		if is_module_enabled(module_id):
			...
			debug_logger.warning("模块未启用：" + module_id)
			return null
