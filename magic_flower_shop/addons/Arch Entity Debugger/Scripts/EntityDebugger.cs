...
public partial class EntityDebugger : Control
{
    ...
    private void RefreshEntityListTree()
    {
        ...
        {
            var types = archetype.Signature.Components.ToArray();
            if (!ArchetypeManager.TryGetArchetypeDisplayName(types, out string archetypeKey))
            {
                stringBuilder.Clear();

                foreach (Arch.Core.Utils.ComponentType type in types)
                {
